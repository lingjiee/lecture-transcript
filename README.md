...
**注意：术语表不会回溯。** 加进去的裁决只对**之后**清洗的稿子生效，
已经清洗完的稿子不会自动更新。攒了一批新词法之后，值得回头把旧稿重跑一遍
（或者手工补上那几处）。这是实际踩到过的：几集稿子里留着 `demonet`、`ajy`
这样的拼法，而术语表里早已裁决它们是 Gemini 和 agy。

想快速回顾、不想重读全文时，再说一句「用 digest 给 <某个>.md 做导读」，
得到一份带时间戳的索引。**逐字稿和导读是两个文件，各守各的契约**：
逐字稿只删不写，用字符级 diff 保证；导读允许生成，用事实核查保证
（它提到的每个数字、英文词、《书名》都必须在逐字稿里有出处）。

三层串起来：导读看全貌 → 逐字稿查原话 → SRT/音频听语气。
...
66 个测试，只用标准库，不需要装 faster-whisper（`transcribe.py` 把模型的 import
...
- **`srt-verify`**：只删应通过、加标点和重新分段不算改写、替换和新增必须被抓出、
  退出码等于问题数、`--whole` 与默认模式的区别；否定词和数字的删除必须被抓出，
  而「非常」的非、「特别」的别、「未来」的未这些不是否定的音节不能误报。
  另有一条测试专门锁住**它拦不住什么**（见下）。
- **复读机检测**：连续两段相同算强调不报，三段起才算幻觉；跨度覆盖整段重复。
...
- **`srt-verify` 拦不住过度删除。** 删除永远是子序列，改写检查判不出来。
  从这块风险里切出了后果最严重的一小块单独守：**否定词和数字的删除会被专门报出来**。
  删一个「不」，「这不是推荐方案」就变成「这是推荐方案」——意思反了，而它仍是严格
  子序列。剩下的过度删除靠 agent 定义里的删除清单约束，没有机器兜底。
- **它也拦不住"用原文出现过的字重新拼出来的假话"。** 因果、程度、立场无法机器校验。
- **`--facts-only` 顺带是个过期检测器。** 逐字稿改了而导读没跟着改，导读里那些
  已经不在正文里的专名会立刻被报出来。这不是设计目标，是子序列检查的副产品，
  但很好用——改完正文重跑一次就知道导读要不要同步。
- **整条链拦不住 ASR 幻觉。** 语音识别凭空生成的内容会作为"原文"进入流水线，
  cleaner 忠实保留、`srt-verify` 报 0 处改动。这条链保证的是**忠实于 ASR**，
  不是忠实于录音。转写脚本会对复读机式的循环幻觉发警告，但那只是幻觉的一种。
